...
module.exports = {
  ...
  updateUser: async (data) => await User.update({ password: data.newPassword, nickname: data.newName, image: data.newImage }, { raw: true, where: { id: data.id } }),
  ...
  getJunctionTableData: async (singlepost_id, gallerypost_id) => await Singlepost_gallerypost.findAll({ raw: true, where: { [Op.and]: [{ singlepost_id }, { gallerypost_id }] } }),
  ...
  mygetSinglepost: async (id) => await Singlepost.findAll({ where: { user_id: id } }),
